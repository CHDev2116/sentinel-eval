...
from sentinel_eval.schemas.audit import LEGACY_SAFE_KEY
...
def parse_audit_response(raw_output):
    ...
    try:
        ...
        parsed["security_status"] = str(loaded.get("security_status", "")).strip() or "Unknown"
        for key in ("risk_score", "confidence", "uncertainty"):
            if key not in loaded:
                continue
            try:
                parsed[key] = float(loaded[key])
...
def parse_loose_json(text):
    """Best-effort JSON parse (e.g. judge score objects); returns dict or error shape."""
    try:
